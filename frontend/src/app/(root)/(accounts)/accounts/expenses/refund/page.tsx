...
const moneyReturns: MoneyReturn[] = [
  {
    id: 1,
    serial: 1,
    date: '08 Jul 2025',
    receiptFor: 'Customer A',
    category: 'Refund',
    account: 'Cash',
    checkNo: '',
    description: 'অতিরিক্ত টাকা ফেরত',
    transactionType: 'রিফান্ড',
    bank: '',
    amount: 2000
  },
  {
    ...
    serial: 2,
    date: '06 Jul 2025',
    receiptFor: 'Customer B',
    category: 'Advance Return',
    account: 'Bank',
    checkNo: 'CHK-008',
    description: 'অগ্রিম ফেরত',
    transactionType: 'অগ্রিম',
    bank: 'DBBL',
    amount: 3000
  }
]
...
const MoneyReturnTable: React.FC<{ data: MoneyReturn[] }> = ({ data }) => {
  return (
            ...
            data.map(row => (
              <TableRow key={row.id} className="hover:bg-muted/50">
                {tableFields.map(field => {
                  ...
                  const value = row[field.key as keyof MoneyReturn]
                  return (
...
export default function MoneyReturnListPage() {
  const [currentPage, setCurrentPage] = useState(1)
  const [rowsPerPage, setRowsPerPage] = useState(10)
  const totalPages = Math.ceil(moneyReturns.length / rowsPerPage)
  const currentData = moneyReturns.slice((currentPage - 1) * rowsPerPage, currentPage * rowsPerPage)
    ...
    <div className="w-full p-4 space-y-6">
      <Card className="w-full p-6 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <h3 className="text-lg font-semibold">টাকা ফেরত লিস্ট</h3>
        </div>

        <div className="flex items-center gap-4">
          <Input type="text" placeholder={tg('search-by', { name: 'কাস্টমার / রিসিপ্ট' })} />
          ...
          <PrintWrapper printTitle="Money Return List" preview={false}>
            <div className="p-6 bg-white">
              <PrintableHeader
                title="Money Return List"
                ...
                accentColor="red-600"
              />
              <MoneyReturnTable data={currentData} />
            </div>
          </PrintWrapper>
        </div>
